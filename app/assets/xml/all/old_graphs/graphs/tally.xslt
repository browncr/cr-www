...
<xsl:stylesheet
	...
	<xsl:param name="tally-colors" select="document('/xml/all/tally_colors.xml')/tally-colors"/>
	...
	<xsl:template match="answers/q">
		...
		<xsl:call-template name="bar">
			...
			<xsl:with-param name="y" select="$y"/>
			<xsl:with-param name="x" select="$text-width + $width1"/>
			<xsl:with-param name="width" select="$width2"/>
		</xsl:call-template>
		<xsl:call-template name="bar">
			<xsl:with-param name="nbr" select="3"/>
			<xsl:with-param name="y" select="$y"/>
			<xsl:with-param name="x" select="$text-width + $width1 + $width2"/>
			<xsl:with-param name="width" select="$width3"/>
		</xsl:call-template>
		<xsl:call-template name="bar">
			<xsl:with-param name="nbr" select="4"/>
			<xsl:with-param name="y" select="$y"/>
			<xsl:with-param name="x" select="$text-width + $width1 + $width2 + $width3"/>
